...
**重要：只为一级小节生成学习材料（如 1.1、1.2），不递归处理更小的子节（如 1.1.2）**
...
agents:
  - name: "分析-章节-{{chapter.num}}"
    ...
    prompt: |
      ...
      ## 当前任务
      请分析以下章节内容，识别所有**一级小节**并生成详细分析。

      ### 重要说明
      - **只处理一级小节**（如 1.1、1.2、1.3），不要处理二级子节（如 1.1.2）
      - 二级子节的内容合并到其所属的一级小节中处理
      - 避免内容切分过细导致冗余
      ...
      请识别所有一级小节，为每个小节生成完整的 Deep Dive 分析。
...
#### 3.3 并行生成章节概览（合并概览与总结）

读取 `references/chapter-overview-template.md`（已合并原概览和总结），**并行为所有章节生成学习概览**：
...
agents:
  - name: "概览-章节-{{chapter.num}}"
    model: "default"
    prompt: |
      你是一个专业的学习材料设计专家。请基于章节内容生成完整的学习概览与总结。
...
读取 `references/exercises-template.md`，**并行为所有章节生成习题与解答**：

```yaml
agents:
  - name: "习题-章节-{{chapter.num}}"
    model: "default"
    prompt: |
      你是一个专业的习题设计专家。请基于章节内容生成完整的习题与解答。

      ### 习题模板
      {{read "references/exercises-template.md"}}
      ...
      ### 章节概览内容
      {{chapter.overview_content}}

      请生成完整的习题与解答，包括：
      1. 基础练习题（概念理解、计算应用）
      2. 综合练习题（多知识点综合、证明题）
      3. 挑战练习题（开放性问题、应用创新题）
      4. 自测模拟卷（选择题、填空题、计算题、证明题、综合应用题）
...
2. `00_概览.md` - 章节概览与总结（已合并）
3. `{{section.num}}_{{section.title}}.md` - 各小节详细分析（仅一级小节）
4. `习题与解答.md` - 习题与详细解答
...
agents:
  - name: "检查-章节-{{chapter.num}}"
    ...
    prompt: |
      ...
      ## 注意
      - 检查只包含一级小节的分析文件（无 1.1.2 等二级子节文件）
      - 检查合并后的概览文件（包含原概览和总结内容）
      - 检查新增的习题与解答文件
...
1. **文件结构完整性**
   - [ ] 概览文件 (00_概览.md) 存在且内容完整（已合并概览+总结）
   - [ ] 所有一级小节文件存在且命名规范（无二级子节文件）
   - [ ] 习题与解答文件 (习题与解答.md) 存在且内容完整
...
3. **模板符合度**
   - [ ] 概览文件包含所有必需章节（学习目标、核心逻辑、概念对比等）
   - [ ] 小节文件包含所有必需章节（背景、概念、定理、示例等）
   - [ ] 习题文件包含多难度分级题目
...
1. **小节层级控制**: 严格只生成一级小节（X.Y）的学习材料，二级子节（X.Y.Z）内容合并到一级小节中处理
2. **并行安全**: 各 Agent 只写入自己的章节目录，避免文件冲突
3. **资源控制**: 监控 API 调用频率，避免触发限制
4. **文件命名**: 章节目录和文件使用中文命名，保持与原始文件一致
5. **公式格式**: 所有数学公式使用 LaTeX 格式
6. **图片处理**: 保留原始内容中的图片引用，确保相对路径正确
7. **增量处理**: 如果输出目录已存在，询问用户是否覆盖或跳过已处理章节
8. **质量优先**: 宁可少处理几个章节，也要确保生成内容的质量
